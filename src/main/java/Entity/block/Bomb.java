...
public class Bomb extends Entity {
    ...
    public static int power_bomb = 0;   // Sức mạnh của quả bom
    private static int power_bomb_down = 0;     // Sức mạng của quả bom theo các hướng
    ...
    public static int is_bomb = 0;      // Xem xem chỗ này có bom chưa //0 no //1 có //2 đang nổ

    public Bomb(int x, int y, Image img) {
        super(x, y, img);
    }

    public static void putBomb() {      //khi người chơi đặt bom
        if (is_bomb == 0 && bomb_number > 0) {
            new SoundManager("sound/put_bombs.wav", "putBomb");
            bomb_number--;
            is_bomb = 1;
            time_bomb = System.currentTimeMillis();//Đấy thời gian hiện tại theo mili giây
            time_tmp = time_bomb;
            int x = player.getX() / 32;
            int y = player.getY() / 32;
            x = Math.round((float)x);
            y = Math.round((float)y);
            bomb = new Bomb(x, y, Sprite.bomb.getFxImage());//Lấy một quả bom
            block.add(bomb);
    ...
    public static void activeBomb() {   // Thay đổi ảnh trong quá trình bom chuẩn bị nổ
        if (swap_active == 1) {
    ...
    public static void createEdge() {   //Tạo biên độ của quả bom theo các hướng
        int i;
        if (Blocked.block_down_bomb(bomb, 0)) {
            edge_down = new Bomb(bomb.getX() / 32, bomb.getY() / 32 + 1, Sprite.bomb_exploded.getFxImage());
            if (power_bomb > 0) {//Nếu sức mạnh quả bom lớn 
                for(i = 1; i <= power_bomb && Blocked.block_down_bomb(bomb, i); ++i) {
                    edge_down.setY(bomb.getY() + 32 + i * 32);//Thêm ở hướng max của nó(Tại biên độ xa nhất có thể đặt hình ảnh biên độ tại đó)
                    ++power_bomb_down;
                }
            }
    ...
    public static void createMiddle() {     //Từ vị trí tâm ra đến biên độ thêm các ảnh để tạo ra một hình ảnh hoàn thiện
        Entity middle;
        int i;
        ...
        block.addAll(list_bomb_middle_width);//Thêm nhưng bom theo hướng ngang để vẽ
        block.addAll(list_bomb_middle_height);//Cái này hướng dọc 
    }

    public static void explosionCenter() {      // Xác định trung tâm của quả bom và các trạng thái của quả bom (liên tục thay đổi tạo ảnh động)
        if (swap_explosion == 1) {
            bomb.setImg(Sprite.bomb_exploded.getFxImage());
            list_kill[bomb.getX() / 32][bomb.getY() / 32] = 4;//Đánh dấu ô này là chết ,có vật thể nào ở ô này là chết
            ...
            if (list_bomb_middle_height.size() > 0) {
                for (Entity e : list_bomb_middle_height) {
                    ...
                    list_kill[e.getX() / 32][e.getY() / 32] = 4;//Các ô cheets ở đoạn giữa
                }
            }
    ...
    private static void checkActive() {     // Check trạng thái của quả bom ,nếu chưa đến thời gian nổ thì sẽ liên tục active để tạo hiệu ứng
        if (is_bomb == 1) {
    ...
    private static void checkExplosion() {      // Xem đã đến lúc nổ chưa
        if (is_bomb == 2) {   //Nổ thôi
            if (System.currentTimeMillis() - time_bomb < 1000L) {  //Tạo hiệu ứng nổ , lấy các góc cạnh , tt và ô giữa cần thiết
            ...
            else {//Lúc này xong rồi thì xóa và đặt lại trạng thái các ô
                is_bomb = 0;
                id_objects[bomb.getX() / 32][bomb.getY() / 32] = 0;//Hết nguy hiểm rồi 
                list_kill[bomb.getX() / 32][bomb.getY() / 32] = 0;
                bomb.setImg(Sprite.transparent.getFxImage());
                if (Blocked.block_down_bomb(bomb, power_bomb_down)) {
                    edge_down.setImg(Sprite.transparent.getFxImage());//Set lại hình ảnh không có bom 
                ...
                block.removeAll(list_bomb_middle_height);//Xóa các vật thể sau khi đã nổ
